        ...
        if dataset == "Implementation periods":
            ...
            def Loading_API():
                ...
                WorldBank_countries['region'] = WorldBank_countries['region'].map({
                    ...
                    'EAS': 'East Asia and Pacific',
                    'NAC': 'North America'})
                WorldBank_countries.rename(
                    columns={"id": "SpatialDim", "incomeLevel": "Income level", "region": "Region"},
                    inplace=True)
                country_list = country_list.merge(WorldBank_countries, how='left', on='SpatialDim')
            ...
            with st.sidebar:
                ...
                if len(df2_group_country.groupby(['grantAgreementNumber'])) != 1:
                    start_year, end_year = st.select_slider(
                        ...
                        options=list(df2_group_country.programStartDate.astype('datetime64[ns]').dt.year.sort_values(
                            ascending=True).unique()),
                        value=(df2_group_country.programStartDate.astype('datetime64[ns]').dt.year.sort_values(
                            ascending=True).min(),
                               df2_group_country.programStartDate.astype('datetime64[ns]').dt.year.sort_values(
                                   ascending=True).max()))
                    # Filtered dataset:
                    df1_filtered_dates = df2_group_country[
                        (df2_group_country.programStartDate.astype('datetime64[ns]').dt.year >= start_year) & (
            ...
            with tab2:
                ...
                col3.metric("Grants renewed once or more",
                            "{} %".format(
                                round(Number_renewed / len(df1_filtered_dates.grantAgreementId.unique()) * 100)))
                ...
                with tab3:
                    ...
                    def genSankey(df, cat_cols=[], value_cols='', title='Sankey Diagram'):
                        ...
                        for i in range(len(cat_cols) - 1):
                            ...
                            sourceTargetDf = sourceTargetDf.groupby(['source', 'target']).agg(
